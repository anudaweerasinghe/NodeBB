...
json_data=$(jq --arg host "$REDIS_HOST" --arg port "$REDIS_PORT" --arg password "$REDIS_PASSWORD" --arg url "$DEPLOYMENT_URL" '.url = $host | .redis.host = $host | .redis.port = $port | .redis.password = $password' <<< "$json_data")
